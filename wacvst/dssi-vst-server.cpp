...
/* todo: we aren't going to need these and if we do we need to make them
   class members */
int m_AudioMasterRequestFd;
int m_AudioMasterResponseFd;
/* todo: we need to make this a class member */
extern bool m_inProcess;
...
public:
    ...
    virtual int		     getProgramName(int, char *);
...
RemoteVSTServer::process(float **inputs, float **outputs, int sampleFrames)
{
	...
	currentSamplePosition += sampleFrames;
	return;
    }
...
//printf("calling pluginReplacing\n");
    m_plugin->processReplacing(m_plugin, inputs, outputs, sampleFrames);
//printf("returning from pluginReplacing\n");
   currentSamplePosition += sampleFrames;
...
void RemoteVSTServer::eff_mainsChanged(int v)
{
    m_plugin->dispatcher(m_plugin, effMainsChanged, 0, v, NULL, 0);
}
...
void
RemoteVSTServer::setSampleRate(float sr)
{
    pthread_mutex_lock(&mutex);
...
int RemoteVSTServer::getProgramName(int p, char *name)
{
   if (debugLevel > 1) {
	...
	int r;
	if (p < 0) {
		r = m_plugin->dispatcher(m_plugin, effGetProgramName, 0, 0, name, 0);
	} else {
		r = m_plugin->dispatcher(m_plugin, effGetProgramNameIndexed, p, 0, name, 0);
	}	
...
{
	int els = readInt(m_controlRequestFd);
	VstEvents *evt = (VstEvents *) malloc(sizeof(VstEvents) + 4*els);
	evt->numEvents = els;
	for (int i = 0; i < els; i++) {
		int evt_size = readInt(m_controlRequestFd);
		evt->events[i] = (VstEvent *) malloc(evt_size);
		tryRead(m_controlRequestFd, evt->events[i], evt_size);
	}
	int v = m_plugin->dispatcher(m_plugin, effProcessEvents, 0, 0, evt, 0);
	for (int i = 0; i < els; i++) {
		free(evt->events[i]);
	}
	free(evt);
...
void RemoteVSTServer::terminateGUIProcess()
{
    if (m_guiFifoFd >= 0) {
	writeOpcode(m_guiFifoFd, RemotePluginTerminate);
	m_guiFifoFd = -1;
...
#endif
{
    ...
    int rv = 0;
    //    std::cerr << "audio master in  process " << opcode << " " << inProcessThread << std::endl;
    switch (opcode) {
...
//	if (debugLevel > 1)
...
#ifdef MASTER_THREAD
	{
		int j = audioMasterIdle;
		write(m_AudioMasterRequestFd, &j, 4);
		read(m_AudioMasterResponseFd, &j, 4);
	}
#endif
	plugin->dispatcher(plugin, effEditIdle, 0, 0, 0, 0);
	plugin->dispatcher(plugin, 53, 0, 0, 0, 0);
	break;
	...
	if (m_inProcess) {
	    int j = audioMasterGetTime;
	    write(m_AudioMasterRequestFd, &j, 4);
	    j = value;
	    write(m_AudioMasterRequestFd, &j, 4);
		read(m_AudioMasterResponseFd, &timeInfo, sizeof(timeInfo));
	}
	rv = (long)&timeInfo;
	break;
...
AudioThreadMain(LPVOID parameter)
{
    ...
    ttcpolicy.period=(uint32_t) (freq*.01);
    ttcpolicy.computation=(uint32_t)(freq*.0005);
    ttcpolicy.constraint=(uint32_t)(freq*.005);
	...
	try {
	    ...
	    cerr << "ERROR: Remote VST plugin communication failure 1" << endl;
	    exiting = true;
	}
    }
...
//!!! better debug level support
	...
	cerr << "ERROR: Remote VST plugin communication failure 2" << endl;
	return 1;
    }
...
//    wclass.hbrBackground = (HBRUSH)GetStockObject(BLACK_BRUSH);
    ...
    while (!exiting) {
		while (!exiting && PeekMessage(&msg, 0, 0, 0, PM_REMOVE)) {
			// fprintf(stderr, "dispatching %d %d\n", msg.message, cnt++);
			DispatchMessage(&msg);
		...
		try {
			...
			cerr << "ERROR: Remote VST plugin communication failure 3" << endl;
			exiting = true;
			break;
		}
